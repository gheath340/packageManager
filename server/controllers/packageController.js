...
const updatePackageCity = async (p, driver1, body, params) => {
  driver1[0].packages = driver1[0].packages.filter(
    (package) => package._id.toString() !== params.id
  );
...
const updateDriverPackages = (p, driver) => {
  const index = driver[0].packages.findIndex(
    (package) => package._id.toString() === p._id.toString()
  );
...
const deletePackage = async (req, res) => {
  ...
  d[0].packages = d[0].packages.filter(
    (package) => package._id.toString() !== req.params.id
  );
